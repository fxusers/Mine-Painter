...
public class EventHandler {
   ...
   @SubscribeEvent
   public void onPreRenderWorld(Pre e) {
   ...
   public void onDrawPaintingPixel(DrawBlockHighlightEvent event) {
      ...
      if (is != null && is.getItem() instanceof PaintTool) {
         ...
         if (w.getBlock(x, y, z) == ModMinePainter.painting.block) {
            ...
            if (mop != null) {
               float[] point = pp.block2painting((float)(mop.hitVec.xCoord - (double)mop.blockX), (float)(mop.hitVec.yCoord - (double)mop.blockY), (float)(mop.hitVec.zCoord - (double)mop.blockZ));
               point[0] = (float)((int)(point[0] * 16.0F)) / 16.0F;
               point[1] = (float)((int)(point[1] * 16.0F)) / 16.0F;
